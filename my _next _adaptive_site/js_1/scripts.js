$("document").ready(function () {
 ...
 $("header h1 i").hover(hightLightPara);
    function hightLightPara() {
        $('header h1 i').slideUp(100);
        $(this).addClass("hightlight_i");
        //$('header h1 i').html("<i class="fa fa-star-o" aria-hidden="true"></i>");
        $('header h1 i').slideDown(100);
        //$("#box_1").html("<h4 id='bcol'>hey you where are you goin</h4>");
    };
      $("header h1 i").on("mouseleave", whenMouseLeaves);

    function whenMouseLeaves() {
        $("header h1 i").removeClass("hightlight_i");
    }
/*$('img.class').hover(
...
/*$('header h1 i').hover(
    function { $('img.destination').attr('src', '../frcst_img/real_man.png'); }, //<тут меняем картинку
    function { $('img.destination').attr('src', '../frcst_img/wooman_1.png'); } //<тут меняем картинку обратно(то есть когда курсор мыши уходит за пределы img.class)
);
*/
/*$("header h1 i").hover(
    function()
    {
        ...
        $(this).src=newsrc+"frcst_img/real_man.png";
    },
    function()
    {
        var q=$(this).src;
        ...
        $(this).src=newsrc+"frcst_img/wooman_1.png";
    })*/
